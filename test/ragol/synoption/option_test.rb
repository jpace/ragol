...
module Synoption
  class OptionTestCase < TestCase    
    def test_init_minimal
      opt = Option.new :limit, '-l', "the number of log entries", nil
    ...
    def test_init_default_value
      opt = Option.new :nombre, '-x', " one two three", 133
      assert_equal 133, opt.value
    end

    def test_init_negate
      opt = Option.new :limit, '-l', "the number of log entries", nil, :negate => [ %r{^--no-?limit} ]
      assert_equal [ %r{^--no-?limit} ], opt.negate
    end

    def test_to_doc
      opt = Option.new :limit, '-l', "the number of log entries", 777, :negate => [ %r{^--no-?limit} ]
      sio = StringIO.new
      opt.to_doc sio
      exp = String.new
    ...
    def test_exact_match
      opt = Option.new :limit, '-l', "the number of log entries", 3
      [ '-l', '--limit' ].each do |val|
    ...
    def test_negative_match
      opt = Option.new :limit, '-l', "the number of log entries", 777, :negate => [ '-L', %r{^--no-?limit} ]
    ...
    def test_regexp_match
      opt = Option.new :revision, '-r', "the revision", nil, :regexp => Regexp.new('^[\-\+]\d+$')
    ...
    def make_xyz_option options = Hash.new
      Option.new :xyz, '-x', "the blah blah blah", nil, options
    end
